...
export class CalendarEventSidebarComponent implements OnInit {
  ...
  addSession() {
    // Check if both date pickers are defined and if they are valid
    if (
      this.startDatePicker &&
      this.endDatePicker &&
      this.startDatePicker.nativeElement.children[0].value &&
      this.endDatePicker.nativeElement.children[0].value
    ) {
      const sessionData = {
        date: this.startDatePicker.nativeElement.children[0].value,
        time: this.endDatePicker.nativeElement.children[0].value,
        location: this.event.extendedProps.location,
        topics: this.event.extendedProps.description,
      };

      // Call the service to add the session
      this._calendarService.addSession(sessionData).subscribe(
        (response) => {
